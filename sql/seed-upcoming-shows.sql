-- Idempotent three-day public schedule extension.
-- Adds a second Lulu screen using the canonical IMAX seat geometry, then
-- schedules an afternoon repeat and a clearly labelled 11:59 PM show.

UPDATE movies
SET poster_url = '/posters/varavu-2026.webp'
...
FROM seat_layouts
WHERE id = 'LAYOUT_LULU_IMAX1_JSON'
ON DUPLICATE KEY UPDATE
  name = VALUES(name),
  screen_side_label = VALUES(screen_side_label),
...
SELECT
  'LAYOUT_LULU_SCREEN2_JSON', seat_id, row_label, row_sort, seat_number, zone_code,
  item_type, display_order, gap_width, is_blocked, accessibility
FROM seat_layout_seats
WHERE layout_id = 'LAYOUT_LULU_IMAX1_JSON'
ON DUPLICATE KEY UPDATE
  row_label = VALUES(row_label),
  row_sort = VALUES(row_sort),
  seat_number = VALUES(seat_number),
  zone_code = VALUES(zone_code),
  item_type = VALUES(item_type),
  display_order = VALUES(display_order),
  gap_width = VALUES(gap_width),
  is_blocked = VALUES(is_blocked),
  accessibility = VALUES(accessibility);

INSERT INTO shows (
  id, movie_id, theatre_id, screen_id, layout_id, show_time, authority_mode, status
)
SELECT CONCAT('SHOW_LULU_EXTRA_', DATE_FORMAT(show_date, '%Y%m%d'), '_1500'),
       'drishyam_3_2026', 'THEATRE_LULU_TVM', 'SCREEN_LULU_2',
       'LAYOUT_LULU_SCREEN2_JSON', TIMESTAMP(show_date, '15:00:00'), 'CENTRAL_AUTHORITY', 'OPEN'
FROM (
...
SELECT CONCAT('SHOW_LULU_MIDNIGHT_', DATE_FORMAT(show_date, '%Y%m%d')),
       'varavu_2026', 'THEATRE_LULU_TVM', 'SCREEN_LULU_2',
       'LAYOUT_LULU_SCREEN2_JSON', TIMESTAMP(show_date, '23:59:00'), 'CENTRAL_AUTHORITY', 'OPEN'
FROM (
  SELECT CURRENT_DATE AS show_date
  UNION ALL SELECT DATE_ADD(CURRENT_DATE, INTERVAL 1 DAY)
  UNION ALL SELECT DATE_ADD(CURRENT_DATE, INTERVAL 2 DAY)
