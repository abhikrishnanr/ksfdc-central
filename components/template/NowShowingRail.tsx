        ...
        {movies.map((movie) => (
          <article className="poster-movie-card" key={movie.id}>
            <Link href={`/movies/${movie.id}`}>
              <div className="poster-frame" style={movie.posterUrl ? { backgroundImage: `url("${movie.posterUrl}")` } : undefined}>
                {isNewMovie(movie.releaseDate) ? <span className="new-badge">New</span> : null}
              </div>
              <div className="poster-card-copy">
                <h3>{movie.title}</h3>
                <p>{[movie.language, movie.genres[0]].filter(Boolean).join(' | ')}</p>
                {movie.certificate ? <span>{movie.certificate}</span> : null}
              </div>
            </Link>
            <Link className="book-now-outline" href={`/movies/${movie.id}/book`}>Book Now</Link>
          </article>
        ))}
      </div>
    </section>
