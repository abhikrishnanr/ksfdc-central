...
export default function HeroCarousel({ movie, show }: { movie: CentralMovieSummary | null; show?: PublicShowtimeSummary | null }) {
  ...
  return (
    ...
    >
      <button className="hero-arrow left" type="button" aria-label="Previous featured movie"><ChevronLeft /></button>
      <div className="hero-movie-copy">
        ...
        <Link className="gold-button" href={href}><Ticket size={19} /> Book tickets <ArrowRight size={18} /></Link>
      </div>
      <button className="hero-arrow right" type="button" aria-label="Next featured movie"><ChevronRight /></button>
      <div className="hero-dots" aria-hidden="true"><span className="active" /><span /><span /><span /><span /></div>
    </section>
  );
}
